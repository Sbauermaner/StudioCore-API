...
class HybridGenreEngine:
    ...
    def resolve(
        ...
        """
        # Handle genre parameter (for pipeline calls: resolve(genre=...,
        # context=...))
        if genre is not None:
            # Signature 2: genre and context (for pipeline)
            # For now, just return the genre (can be enhanced later)
            return genre
    ...
    def collect_signals(
        ...
        if folk_ballad_candidate and isinstance(folk_ballad_candidate, dict):
            ...
            if genre:
                signals[genre] = (
                    signals.get(genre, 0.0) + confidence * keyword_weight * 0.5
                )

        # 3. Road narrative - using threshold from config
        if road_narrative_score > road_narrative_min:
            signals["dark country rap ballad"] = (
                signals.get("dark country rap ballad", 0.0) + road_narrative_score * 0.3
            )
        ...
        if hiphop_factor > 0.2:
            signals["hiphop"] = (
                signals.get("hiphop", 0.0) + hiphop_factor * text_weight * 0.83
            )  # 0.3 * 0.83 ≈ 0.25
        ...
        epic = (
            float(emotion_profile.get("epic", 0.0) or 0.0)
            if isinstance(emotion_profile, dict)
            else 0.0
        )
        if cinematic_factor > 0.2 or epic > epic_threshold:
            signals["cinematic"] = (
                signals.get("cinematic", 0.0)
                + max(cinematic_factor, epic) * text_weight * 0.83
            )
        ...
        for cmd in commands or []:
            if cmd.get("type") == "genre" and cmd.get("value"):
                ...
                if cmd_genre not in ("auto", "unknown", ""):
                    signals[cmd_genre] = (
                        signals.get(cmd_genre, 0.0) + command_boost * 0.4
                    )  # 0.25 * 0.4 = 0.1
    ...
    @staticmethod
    def resolve_hybrid_genre(
        signals_data: dict, user_override: Optional[str] = None
    ) -> Optional[str]:
        """
