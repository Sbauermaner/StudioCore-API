...
EMOTION_COLOR_MAP: Dict[str, List[str]] = {
    ...
    "conflict": [
        KEY_COLOR_PALETTE["orange - red"],
        KEY_COLOR_PALETTE["amber"],
        KEY_COLOR_PALETTE["crimson"],
    ],
...
class ColorEngineAdapter:
        ...
        if pain <= LOW_EMOTION_TLP_PAIN_MAX and truth >= LOW_EMOTION_TLP_TRUTH_MIN:
            ...
            if self._is_low_emotion_profile(emo_profile):
                ...
                return ColorResolution(
                    colors=NEUTRAL_COLOR_WAVE, source="neutral_profile"
                )
    ...
    def _resolve_hybrid_colors(
        self, genre_label: str, base_colors: List[str], style_payload: Dict[str, Any]
    ) -> List[str] | None:
        """
    ...
    def _is_low_emotion_profile(self, emotions: dict) -> bool:
        ...
        # Низкая боль / негатив при ощутимом мире → нейтральный / спокойный
        # профиль
...
def blend(color1: str, color2: str, factor: float) -> str:
    ...
    rgb_result = tuple(
        int(r1 * (1 - factor) + r2 * factor) for r1, r2 in zip(rgb1, rgb2)
    )
    ...
    for i in range(len(colors) - 1):
        ...
        if i < len(colors) - 1:
            mixed = blend(
                colors[i],
                colors[i + 1],
                weights[i + 1] if i + 1 < len(weights) else 0.5,
            )
...
def saturate(color: str, factor: float) -> str:
    ...
    gray = sum(rgb) // 3
    rgb_result = tuple(int(c + (c - gray) * factor) if c != gray else c for c in rgb)
