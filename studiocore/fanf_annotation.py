...
class FANFAnnotationEngine:
    ...
    def build_annotations(
        ...
        color_wave = (
            analysis.get("color", {}).get("wave")
            if isinstance(analysis.get("color"), dict)
            else None
        )
    ...
    def _should_add_choir(self, text: str, emotion: Dict[str, Any]) -> bool:
        ...
        if isinstance(emotion, dict):
            profile = (
                emotion.get("profile", {})
                if isinstance(emotion.get("profile"), dict)
                else {}
            )
            sadness_score = float(
                profile.get("sadness") or emotion.get("sadness") or 0.0
            )
    ...
    def _build_cinematic_header(
        ...
        key = (
            _safe(tonality.get("mode"), "auto")
            if isinstance(tonality, dict)
            else "auto"
        )
        anchor = (
            _first(tonality.get("section_keys", []), "C")
            if isinstance(tonality, dict)
            else "C"
        )
        freq = (
            _safe(tlp.get("base_hz", tlp.get("base_frequency", 432)), "432")
            if isinstance(tlp, dict)
            else "432"
        )
        consciousness = _safe(
            tlp.get("consciousness_level", tlp.get("consciousness", 0.72)), "0.72"
        )
    ...
    def _build_resonance_header(
        self, instrumentation: Dict[str, Any], choir_active: bool
    ) -> str:
        palette = (
            instrumentation.get("palette") if isinstance(instrumentation, dict) else []
        )
        primary_instruments = ", ".join(palette) if palette else "adaptive blend"
        fx = (
            instrumentation.get("selection", {}).get("fx")
            if isinstance(instrumentation.get("selection", {}), dict)
            else None
        )
        fx_block = (
            fx or "bells, whispers, stone hall" if choir_active else "micro fx & air"
        )
        atmosphere = (
            instrumentation.get("emotion", {}).get("atmosphere")
            if isinstance(instrumentation.get("emotion"), dict)
            else None
        )
        texture = (
            instrumentation.get("voice", {}).get("style")
            if isinstance(instrumentation.get("voice"), dict)
            else "solo voice"
        )
    ...
    def _build_sections(
        ...
        bpm_val = _safe(bpm.get("estimate"), "120") if isinstance(bpm, dict) else "120"
        modal_shifts_raw = (
            tonality.get("modal_shifts") if isinstance(tonality, dict) else []
        )
        modal_shifts = (
        ...
        emotion_profile = (
            emotion.get("profile", {}) if isinstance(emotion, dict) else {}
        )
        intensity_raw = emotion.get("curve", []) if isinstance(emotion, dict) else []
        intensity = (
            list(intensity_raw.values())
            if isinstance(intensity_raw, dict)
            else list(intensity_raw or [])
        )
        ...
        for idx, section in enumerate(list(sections or [])):
            ...
            tone = (
                _first(modal_shifts[idx:idx + 1], "stable")
                if modal_shifts
                else "stable"
            )
            height = round(intensity[idx] * 100) if idx < len(intensity) else 50
            color_hint = (
                _first(emotion_profile.keys(), "emotion blend")
                if emotion_profile
                else "emotion blend"
            )
            descriptors.append(
                f"[{label}: Rhythm~{bpm_val} | Mode: {tone} | Intensity: {height}% | ColorWave: {color_hint}]"
            )

        if not descriptors:
            descriptors.append(
                "[Intro: Adaptive entry | Rhythm~120 | Mode: stable | Intensity: 50% | ColorWave: neutral]"
            )
        return descriptors
    ...
    def _compose_annotation(
        ...
        zero_map = (
            zero_pulse.get("structure_hint") if isinstance(zero_pulse, dict) else None
        )
        zero_line = f"ZeroPulse: {_safe(zero_map, 'none')}"
        resonance_map = (
            f"Resonance map: {_safe(tonality.get('modal_shifts', []), 'stable')}"
        )
        rde_line = (
            f"RDE: {_safe(rde.get('state', rde.get('dominant_axis', 'balanced')))}"
        )
    ...
    def _build_ui_annotation(
        ...
        sections_block = "\n".join(sections)
        choir_state = (
            "with emotional choir lift" if choir_active else "intimate vocal focus"
        )
        return (
