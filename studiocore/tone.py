...
class ToneSyncEngine:
    ...
    def _select_key_color(self, key: str, cf: float = 0.0) -> str:
        ...
        if cf > 0.5:
            blend_target = (
                ...
                if base_color
                in (KEY_COLOR_PALETTE["red"], KEY_COLOR_PALETTE["orange - red"])
    ...
    def colors_for_primary(
        self, emo: Dict[str, float], tlp: Dict[str, float], key: str = "auto"
    ) -> Dict[str, Any]:
        cf = tlp.get("conscious_frequency", 0.0)
        ...
        harmony_score = round(
            min(1.0, (tlp.get("love", 0) + tlp.get("truth", 0)) / 2), 3
        )
        brightness = round(0.5 + cf / 2, 2)
        contrast = round(abs(tlp.get("love", 0) - tlp.get("pain", 0)), 2)
        signature_id = (
            f"{color[:2]}-{int(resonance)}-{temp[:2]}-{int(harmony_score * 100)}"
        )
    ...
    def detect_key(self, text: str) -> Dict[str, Any]:
        ...
        if matches:
            ...
            if base_note in self.KEY_STEPS or base_note.replace("#", "") in [
                "C",
                "D",
                "E",
                "F",
