...
def normalize_text_preserve_symbols(text: str) -> str:
    # Приводим перевод строк и убираем только лишние пробелы
    text = text.replace("\r\n", "\n").replace("\r", "\n")
    # Сохраняем структуру строк; не схлопываем \n
    # Удаляем хвостовые пробелы у строк
    lines = [re.sub(r"[ \t]+", " ", ln).rstrip() for ln in text.split("\n")]
    return "\n".join(lines).strip()

def extract_sections(text: str) -> List[Dict[str, Any]]:
    """Выделяет логические секции по тегам [Verse], [Chorus], [Bridge], [Tagelharpa + throat singing] и т.п."""
...
class AutoEmotionalAnalyzer:
    ...
    def analyze(self, text: str) -> Dict[str, float]:
        ...
        # Пунктуационная добавка к «интенсивности» эмоций
        punct_intensity = 0.0
        for ch in s:
        ...
        # Распределяем добавку в «anger/epic/joy» частично
...
class LyricMeter:
    ...
    def bpm_from_density(self, text: str) -> int:
        ...
        # корректируем за счет пунктуации
...
class StyleMatrix:
    ...
    def recommend(self, emo: Dict[str, float], tlp: Dict[str, float],
        ...
        # если найдены музыкальные подсказки в секциях, добавим краткую ссылку
        tag_hints = []
        if sections:
            for s in sections:
                tag = s.get("tag", "")
...
def soft_trim(text: str, max_len: int) -> str:
    ...
    if len(txt) <= max_len:
        return txt
    # приоритетное удаление хвостовых блоков
...
class StudioCore:
    ...
    def analyze(self, lyrics: str, prefer_gender: str = "auto",
        ...
        # если в тегах есть подсказки инструментов — аккуратно расширяем
