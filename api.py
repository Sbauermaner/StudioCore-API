...
class AnalyzeRequest(BaseModel):
    ...
    text: str = Field(..., description="Текст для анализа", min_length=1, max_length=16000)
    preferred_gender: Optional[str] = Field("auto", description="Предпочтительный пол вокала")
    ...
    def validate_gender(cls, v):
        if v not in ["auto", "male", "female", "neutral", "mixed"]:
            raise ValueError("preferred_gender must be one of: auto, male, female, neutral, mixed")
        return v
