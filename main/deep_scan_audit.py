...
class DeepScanAuditor:
    ...
    def __init__(self, base_path: Path):
        ...
        self.results: List[Tuple[str, str, str]] = []  # (file, status, comment)
    ...
    def check_integrity(self, content: str, file_path: Path) -> Optional[str]:
        ...
        try:
            ...
            # In Produktion würde man einen vollständigen Scope-Tracker verwenden
            ...
            try:
                ...
                if "def resolve" in content:
                    ...
                    if "text_input" in content and "genre" in content:
                        self.results.append(("studiocore/hybrid_genre_engine.py", OK, 
                                           "resolve() Methode vorhanden mit beiden Signaturen"))
                    else:
                        self.results.append(("studiocore/hybrid_genre_engine.py", WARNING,
                                           "resolve() vorhanden, aber möglicherweise unvollständig"))
                else:
                    self.results.append(("studiocore/hybrid_genre_engine.py", ERROR,
        ...
        if emotion_path.exists():
            try:
                ...
                if top_level_imports:
                    self.results.append(("studiocore/emotion.py", WARNING,
                    ...
                    if "from .tlp_engine import" in content:
                        ...
                        if "def " in content[:content.find("from .tlp_engine import")]:
                            self.results.append(("studiocore/emotion.py", OK,
                ...
                self.results.append(("studiocore/emotion.py", ERROR,
                                   f"Fehler beim Prüfen: {e}"))
            ...
            if module_path.exists():
                try:
                    ...
                    if len(content) > 50:  # Nicht leer
                        self.results.append((f"studiocore/{module_name}", OK,
                                           "Datei existiert und ist nicht leer"))
                    else:
                        self.results.append((f"studiocore/{module_name}", WARNING,
                                           "Datei existiert, aber sehr kurz (möglicherweise leer)"))
                except Exception as e:
                    self.results.append((f"studiocore/{module_name}", ERROR,
    ...
    def generate_report(self) -> str:
        ...
        audit_results = [r for r in self.results if any(keyword in r[0] for keyword in 
                        ['hybrid_genre_engine', 'emotion.py', 'universal_frequency_engine', 
                         'hybrid_instrumentation_layer', 'neutral_mode_pre_finalizer'])]
